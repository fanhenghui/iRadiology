...
window.AjaxPanel = AjaxPanel = {
    ...
    reload: function(panel, url) {
        if (!panel.location)
            return;
        var requestUrl = url ? url : window.location;
        ...
        new Ajax.Request(requestUrl, {
            ...
            onSuccess: function (transport) {
                var dIndex = transport.responseText.indexOf("|")
                panel.location = transport.responseText.substring(0, dIndex);
                AjaxPanel._update(panel, transport.responseText.substring(dIndex + 1));
    ...
    _bind: function(panel) {
        ...
        if(panel.onrefresh && panel.wereRunned) {
            panel.onrefresh.apply(panel);
        }
        if (!panel.wereRunned)
            panel.wereRunned = true;
    ...
    _submitForm: function(form) {
        if (form != null) {
            ...
            new Ajax.Request(action, {
                ...
                onSuccess: function(transport) {
                    var dIndex = transport.responseText.indexOf("|")
                    form.panel.location = transport.responseText.substring(0, dIndex);
                    AjaxPanel._update(form.panel, transport.responseText.substring(dIndex + 1));
                    AjaxPanel._hideProgress();
                    if (Prototype.Browser.IE)
                        document.fireEvent("onreadystatechange");
                }
    ...
    _linkClick: function(event) {
        ...
        new Ajax.Request(getUrl, {
            ...
            onSuccess: function(transport) {
                var dIndex = transport.responseText.indexOf("|")
                sender.panel.location = transport.responseText.substring(0, dIndex);
                AjaxPanel._update(sender.panel, transport.responseText.substring(dIndex + 1));
                AjaxPanel._hideProgress();
                if (Prototype.Browser.IE)
                    document.fireEvent("onreadystatechange");
            }
